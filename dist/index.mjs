...
async function extractFace(imageBuffer, _mimetype, _model, opts) {
  ...
  try {
    ...
    const extended = needsExtend ? await sharp(oriented).extend({ top: extT, bottom: extB, left: extL, right: extR, background: "#FFFFFF" }).toBuffer() : oriented;
    const photo = await sharp(extended).extract({ left: cropLeft, top: cropTop, width: cropW, height: cropH }).resize(256, 256).jpeg({ quality: 92 }).toBuffer();
...
async function detectAndSplitCompositeCedulaV3(imageBuffer, mimetype, model = "gemini") {
  ...
  try {
    frontBuf = await sharp(frontBuf).trim(trimOpts).toBuffer();
  } catch {
  }
  try {
    backBuf = await sharp(backBuf).trim(trimOpts).toBuffer();
  } catch {
  }
...
export { configure, detectCedulaSide, extractCedulaFace, isUnreadable, splitCompositeCedula };
