...
var toAiModel = (m) => m === "gemini" ? "GEMINI" : "ANTHROPIC";
...
async function extractFace(imageBuffer, _mimetype, _model, opts) {
  const log = getLogger();
  const oriented = await sharp__default.default(imageBuffer).rotate().toBuffer();
  const metadata = await sharp__default.default(oriented).metadata();
  ...
  try {
    ...
    const extended = needsExtend ? await sharp__default.default(oriented).extend({ top: extT, bottom: extB, left: extL, right: extR, background: "#FFFFFF" }).toBuffer() : oriented;
    const photo = await sharp__default.default(extended).extract({ left: cropLeft, top: cropTop, width: cropW, height: cropH }).resize(256, 256).jpeg({ quality: 92 }).toBuffer();
...
async function cropRegion(buffer, bbox, imgW, imgH) {
  ...
  return sharp__default.default(buffer).extract({ left, top, width, height }).toBuffer();
}
async function detectAndSplitCompositeCedulaV3(imageBuffer, mimetype, model = "gemini") {
  const metadata = await sharp__default.default(imageBuffer).metadata();
  const imgW = metadata.width || 0;
  const imgH = metadata.height || 0;
  if (!imgW || !imgH) return null;
  const aiModel = toAiModel(model);
  ...
  try {
    frontBuf = await sharp__default.default(frontBuf).trim(trimOpts).toBuffer();
  } catch {
  }
  try {
    backBuf = await sharp__default.default(backBuf).trim(trimOpts).toBuffer();
  } catch {
  }
...
function isUnreadable(r) {
  return r != null && r.unreadable === true;
}
async function splitCompositeCedula(file, mimetype, model = "gemini") {
  let renderedBuffer;
  let renderedMimetype;
  let renderedExtension;
...
async function extractCedulaFace(file, mimetype) {
  let imageBuffer = null;
...
exports.extractCedulaFace = extractCedulaFace;
exports.isUnreadable = isUnreadable;
exports.splitCompositeCedula = splitCompositeCedula;
